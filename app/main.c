...
void send_fan_info(unsigned int request_task) {
   char *humidity = num_to_string((unsigned int)get_humidity());
   char *temperature = num_to_string((unsigned int)get_temperature());
...
char *float_to_string(float number) {
   unsigned int first_part = (unsigned int) number;
   float second_part = number - (float)first_part;
   char *first_part_string = num_to_string(first_part);

   if (second_part > 0) {
      usart_framing_errors_counter_g++;
   }

   return first_part_string;
}
